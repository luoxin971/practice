...
/**
 * https://gist.github.com/jayfella/00a328b2dbdf6304078a821143b7aef7 <br>
 * https://hub.jmonkeyengine.org/t/isosurface-mesh-simplifier/41046 <br>
 * 基于 jme3 的曲面简化
 *
 * @author luoxin
 * @since 2022/5/10
...
public class SimplifyMesh {
  ...
  class SymetricMatrix {
    ...
    // Determinant
    final double det(
        int a11, int a12, int a13, int a21, int a22, int a23, int a31, int a32, int a33) {
  ...
  private void readJmeMesh() {
    ...
    // position
    Vector3f[] meshVerts =
        BufferUtils.getVector3Array(this.inMesh.getFloatBuffer(VertexBuffer.Type.Position));
    meshInVerts = meshVerts;

    // normal 法线
    // meshInNorms =
    // BufferUtils.getVector3Array(this.inMesh.getFloatBuffer(VertexBuffer.Type.Normal));

    // IndexBuffer 的 size 似乎与 vertex 的 size 一致
    ...
    for (int i = 0; i < meshIndices.length; i += 3) {
      ...
      // triangle 存的是 vertex 的 index
      final Triangle t =
          new Triangle(meshIndices[index++], meshIndices[index++], meshIndices[index++]);

      triangles.add(t);

      // ref 存的是 triangle 的 index，以及 vertex 的 index
   ...
   *
   * @param target_percent the amount in percent to attempt to achieve. For example: 0.25f would
   *     result in creating a mesh with 25% of triangles contained in the original.
   * @param agressiveness sharpness to increase the threshold. 5..8 are good numbers. more
   *     iterations yield higher quality. Minimum 4 and maximum 20 are recommended.
   * @param complexNormals Whether or not to generate computationally expensive normals, or just use
   *     the face normal.
   */
  ...
  private boolean flipped(
      final Vector3f p, final int i1, final Vertex v0, final Vector<Boolean> deleted) {
    for (int k = 0; k < v0.tcount; k++) {
      ...
      final Vector3f n = new Vector3f(d1).crossLocal(d2).normalizeLocal();
    ...
    if (iteration == 0) {
      ...
      triangles.forEach(
          t -> {
            Vector3f[] p =
                new Vector3f[] {
                  vertices.get(t.v[0]).p, vertices.get(t.v[1]).p, vertices.get(t.v[2]).p,
                };

            Vector3f n = p[1].subtract(p[0]).crossLocal(p[2].subtract(p[0])).normalizeLocal();
            ...
            for (int j = 0; j < 3; j++) {
              vertices
                  .get(t.v[j])
                  .q
                  .set(vertices.get(t.v[j]).q.add(new SymetricMatrix(n.x, n.y, n.z, -n.dot(p[0]))));
            }
          });
  ...
  private double vertex_error(
      final SymetricMatrix q, final double x, final double y, final double z) {
    return q.getValue(0) * x * x
        + 2 * q.getValue(1) * x * y
        + 2 * q.getValue(2) * x * z
        + 2 * q.getValue(3) * x
        + q.getValue(4) * y * y
  ...
  private Mesh createSimplifiedMesh(boolean complexNormals) {
    ...
    // Vector3f[] normArray =
    //     complexNormals ? normalizeMesh(triangles, vertices) : new Vector3f[vertArray.length];
  ...
  public static void main(String[] args) throws Exception {
    Collada collada = Collada.readFile("/Users/luoxin/Documents/xk_dev/dae_file/mall.dae");
    for (int i = 0; i < collada.getLibraryGeometries().getGeometries().size(); i++) {
      com.dddviewr.collada.geometry.Mesh selfMesh =
          collada.getLibraryGeometries().getGeometries().get(i).getMesh();
      float[] positionData = selfMesh.getPositionData();
      int[] index = selfMesh.getPrimitives().get(0).getData();
      float[] normalData = selfMesh.getNormalData();
      if (normalData == null) {
        System.out.println("Continue!!");
        continue;
      }
      Mesh mesh = new Mesh();
      mesh.setBuffer(VertexBuffer.Type.Position, 3, positionData);
      mesh.setBuffer(VertexBuffer.Type.Index, 3, index);
      mesh.setBuffer(VertexBuffer.Type.Normal, 3, normalData);
      Mesh test = new SimplifyMesh(mesh).simplify(0.5f, 6, false);
      int tc = test.getTriangleCount();
      int mc = mesh.getTriangleCount();
      System.out.println(tc + " " + mc);
    }

    // for (int i = 0; i < mesh.getTriangleCount(); i++) {
    //   com.jme3.math.Triangle tri = new com.jme3.math.Triangle();
    //   mesh.getTriangle(i, tri);
    //   Coordinate c1 = new Coordinate(tri.get1().x, tri.get1().y);
    //   Coordinate c2 = new Coordinate(tri.get2().x, tri.get2().y);
    //   Coordinate c3 = new Coordinate(tri.get3().x, tri.get3().y);
    //   Polygon polygon2d = XkPolygonUtil.createPolygon2d(c1, c2, c3, c1);
    //   System.out.println(polygon2d);
    // }

    Box box = new Box(6, 6, 6);
    // box.clearBuffer(VertexBuffer.Type.Normal);
    // printMesh(box);
    //
    // mesh.setBuffer(VertexBuffer.Type.Position, 3,
    // box.getFloatBuffer(VertexBuffer.Type.Position));
    // mesh.setBuffer(
    //     VertexBuffer.Type.Index,
    //     3,
    //     box.getIndexBuffer().getFormat(),
    //     box.getIndexBuffer().getBuffer());
    // printMesh(mesh);
    // Mesh simplify = new SimplifyMesh(box).simplify(0.5f, 6, false);
    // Mesh simplify2 = new SimplifyMesh(mesh).simplify(0.5f, 6, false);
    // DAENode load = DAELoader.create().load(new FileInputStream("xkool/resources/abc.dae"));
    // System.out.println(box.center);
    // System.out.println("---------------------------------------");
    // printMesh(simplify);
  }

  protected static final Mesh createMesh() {
    Mesh mesh = new Mesh();
    // if( mesh == null ) {
    //   mesh = mesh;
    // }
    ...
    int size = 4;
    mesh.setBuffer(VertexBuffer.Type.Position, 3, BufferUtils.createFloatBuffer(size * 4 * 3));
